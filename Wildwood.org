...
\definepapersize [biggy] [width=600mm,height=300mm]
...
def drawdoorup (expr p, offset, length, thickness) =
  ...
  dimension((point 0 of p)--((point 0 of p)+(0,offset)), (10, 0));
enddef;
...
def dimension(expr p, offset) =
  ...
  l := arclength p;
  if (l mod 12 = 0): 
    pic := thelabel("\small $" & decimal(l / 12) & "' $", point 1/2 of p shifted offset);
  else:
    pic := thelabel("\small $" & decimal(floor(l / 12)) & "'" & decimal(l mod 12) & "''$", point 1/2 of p shifted offset);
  fi
  unfill bbox pic;
