...
def drawdoorup (expr p, offset, length, thickness) =
  ...
  undraw (p+(0,offset-(thickness/2)))--(p+(0,offset+length-thickness));
enddef;

def drawdoorright (expr p, offset, length, thickness) =
  pickup pencircle scaled thickness;
  undraw (p+(offset-(thickness/2),0))--(p+(offset+length-thickness,0));
enddef;
...
drawdoorup(point 1 of livingroom, 49, 72, 12);
drawdoorright(point 0 of livingroom, 49, 48, 12);
drawdoorup(point 1 of garage, 16, 36, 12);
drawdoorleft(point 4 of kitchen, 6, 36, 12);
