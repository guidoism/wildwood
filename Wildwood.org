...
def dimension(expr p, offset) = 
  ...
  l := arclength p;
  if (l mod 12 = 0): 
    pic := thelabel("\ss " & decimal(l / 12) & "\prime", point 1/2 of p shifted offset);
  else:
    pic := thelabel("\ss " & decimal(floor(l / 12)) & "\prime" & decimal(l mod 12) & "\prime\prime", point 1/2 of p shifted offset);
  fi
  unfill bbox pic;
